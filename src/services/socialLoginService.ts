...
async function kakaoLoginService(authCode: string, deviceId: string) {
   ...
   const userInfo = await getKakaoUserInfoService(access_token);
   ...
   if (isExistedUser) {
      ...
      await userRepo.updateRefreshToken(user.id, refresh_token);
      return { access_token, refresh_token };
   }
}
...
async function getKakaoUserInfoService(access_token: string) {
   const data = await axios({
      method: 'GET',
